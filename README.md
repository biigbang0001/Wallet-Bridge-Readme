...
**WRAP (FIX → wFIX):**
1. You send your FIX to the bridge address
2. The bridge verifies the transaction (6 confirmations)
3. The daemon mints equivalent wFIX on BSC
4. The wFIX arrives at your BSC address

**UNWRAP (wFIX → FIX):**
1. You burn your wFIX via the smart contract
2. The bridge detects the burn
3. The daemon sends native FIX from its reserve
4. The FIX arrives at your FIX address
...
| 🔐 Private Key WIF | Base58 | `5HueCGU8rMjxEXx...` |
...
|--------|---------|-----------------|
| **WIF** | `5HueCGU8rMjx...` | 51-52 characters, Base58 |
| **HEX** | `e8f32e723dec...` | 64 hexadecimal characters |
| **xprv** | `xprv9s21ZrQH...` | Extended Private Key, full HD |

#### Differences:

| Type | Generated addresses | HD? |
|------|---------------------|-----|
| WIF / HEX | Single address only | No |
| xprv | All addresses (Bech32, Taproot, BSC) | Yes ✅ |
...
│                                                                 │
│  On average: a few minutes to 1+ hour depending on network      │
│                                                                 │
...
│                                                                 │
│  1. FIX network fee      - a few satoshis                       │
│     └─ Paid to FIX miners                                       │
│                                                                 │
│  2. Bridge fee           - 1%                                   │
│     └─ Goes to bridge operator                                  │
│                                                                 │
│  3. BNB gas fee          - ~0.00002 BNB                         │
│     └─ Paid for BSC transaction                                 │
│                                                                 │
│  ─────────────────────────────────────────                      │
│  wFIX received:           ~XX.XX wFIX                           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│                                                                 │
│  1. Bridge fee           - 1%                                   │
│     └─ Deducted during burn                                     │
│                                                                 │
│  2. BNB gas fee          - ~0.00002 BNB                         │
│     └─ Paid for burn TX on BSC                                  │
│                                                                 │
│  ─────────────────────────────────────────                      │
│  FIX received:            ~XX.XX FIX                            │
│                                                                 │
...
| Fee | WRAP | UNWRAP |
|-----|------|--------|
| Bridge fee | 1% FIX | 1% wFIX |
| FIX network fee | You | Bridge |
| BSC gas | ~0.00002 BNB | ~0.00002 BNB |
...
│  □ Keep enough BNB for gas (~0.001 BNB min)                     │
...
> A: Variable. FIX is a SHA-256 blockchain where time between blocks depends on network hashrate. It can take a few minutes to over an hour.
...
**Q: Why is unwrap faster?**
> A: BSC blocks are ~3 seconds and processing is faster than waiting for 6 FIX confirmations.
...
│  2. Recommended minimum: 0.001 BNB                              │
│  3. Buy BNB on an exchange (Binance, etc.)                      │
└─────────────────────────────────────────────────────────────────┘
```
...
│  • If confirmed (6/6) but no wFIX after 1h → contact support    │
│  • Confirmation time varies based on network hashrate           │
└─────────────────────────────────────────────────────────────────┘
```
